...
class ImageReward(nn.Module):
    ...
    def score_image_list(self, prompt, generations_list):
        n = len(generations_list)
        text_input = self.blip.tokenizer(prompt, padding='max_length', truncation=True, max_length=35, return_tensors="pt").to(self.device)

        # preprocess and stack all images into one batch
        images = []
        for generation in generations_list:
            if isinstance(generation, Image.Image):
                pil_image = generation
            elif isinstance(generation, str) and os.path.isfile(generation):
                pil_image = Image.open(generation)
            else:
                raise TypeError(r'This image parameter type has not been supportted yet. Please pass PIL.Image or file path str.')
            images.append(self.preprocess(pil_image))
        images = torch.stack(images, dim=0).to(self.device)  # [N, C, H, W]

        # batch visual encode
        image_embeds = self.blip.visual_encoder(images)  # [N, seq_len, dim]

        # expand text inputs to match batch size
        image_atts = torch.ones(image_embeds.size()[:-1], dtype=torch.long).to(self.device)
        text_ids   = text_input.input_ids.expand(n, -1)
        text_mask  = text_input.attention_mask.expand(n, -1)

        # batch text encode with cross-attention over images
        ...
        txt_features = text_output.last_hidden_state[:, 0, :].float()  # [N, feature_dim]
        rewards = self.mlp(txt_features)  # [N, 1]
        rewards = (rewards - self.mean) / self.std
