...
class NegtomeArgs:
    ...
    push_all: bool = True # whether to push away from only the frist image or everything 
...
def negtome(hidden_states_orig, alpha=0.5, threshold=0.7, dropout=0., mask=None, push_all=False):
    """
    ...
    if push_all:
        # modified implementation where we push away from all frames
        flat_hidden_states_orig = hidden_states_orig.reshape(num_frames * num_tokens, dim) # (B*N, D)
        flat_hidden_states_norm = F.normalize(flat_hidden_states_orig, dim=-1)
        sim = flat_hidden_states_norm @ flat_hidden_states_norm.T
        frame_ids = torch.arange(num_frames, device=hidden_states_orig.device).repeat_interleave(num_tokens)  # [0,0,...,1,1,...,2,2,...]
        same_frame_mask = frame_ids.unsqueeze(1) == frame_ids.unsqueeze(0)  # (B*N, B*N)
        sim.masked_fill_(same_frame_mask, -float('inf'))
        max_sim, argmax_ids = torch.max(sim, dim=-1)
        flat_ref = flat_hidden_states_orig[argmax_ids]  # (B*N, D)
        mask = (max_sim > threshold).unsqueeze(-1)  # (B*N, 1)
        if dropout > 0.:
            dropout_mask = torch.rand(num_frames * num_tokens, 1, device=hidden_states_orig.device) > dropout
            mask = mask * dropout_mask
        # count number of tokens to be merged for logging/debugging
        # num_merged_tokens_percentage = 100 * mask.sum().item() / mask.shape[0]
        # print(f"{num_merged_tokens_percentage:.2f}% of tokens are merged")
        flat_merged = torch.where(mask, (1 + alpha) * flat_hidden_states_orig - alpha * flat_ref, flat_hidden_states_orig)
        # flat_merged = torch.where(mask, flat_hidden_states_orig, flat_hidden_states_orig)
        merged_hidden_states = flat_merged.reshape(num_frames, num_tokens, dim)
    else:
        # original negtome implementation where we only push away from the first frame (which is treated as reference) 
        ...
        if self.use_negtome and negtome_args is not None:
            ...
            if negtome_args['use_negtome']:
                if negtome_args['timestep']<=negtome_args['merging_t_start'] and negtome_args['timestep']>=negtome_args['merging_t_end']:
                    ...
                    hidden_states_img = negtome(hidden_states_img, alpha=alpha, threshold=negtome_args['merging_threshold'], \
                                                dropout=negtome_args['merging_dropout'], mask=mask, push_all=negtome_args['push_all'])
...
# FluxNegToMePipeline
##########################################################################
class FluxNegToMePipelineResample(DiffusionPipeline, FluxLoraLoaderMixin):
    r"""
    ...
    def __call__(
        ...
        start_step: int = 0,    # starting timestep for generation
        end_step: Optional[int] = None, # ending timestep for generation
    ):
        r"""
        ...
        if negtome_args.get('use_negtome', None):
            print('Token Repel is enabled')
        ...
        # Slice timesteps for partial denoising (SoP)
        _end_step = end_step if end_step is not None else len(timesteps)
        timesteps = timesteps[start_step:_end_step]
        # Reset step_index so _init_step_index() re-derives it from the first timestep value
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            progress_bar.update(start_step)  # advance bar to start_step so it reads start→end
            for i, t in enumerate(timesteps):
                ...
                # debug: print when NegToMe is actually active this step
                # _ntm = joint_attention_kwargs['negtome_args']
                # if (_ntm.get('use_negtome', False)
                #         and float(t) <= _ntm.get('merging_t_start', 1000)
                #         and float(t) >= _ntm.get('merging_t_end', 900)):
                #     print(f"  use negtome at t={start_step + i}  (timestep={float(t):.1f})")
        ...
        if output_type == "latent":
            ...
            # last_x0_pred = self._unpack_latents(last_x0_pred, height, width, self.vae_scale_factor)
            # last_x0_pred = (last_x0_pred / self.vae.config.scaling_factor) + self.vae.config.shift_factor
            # image = self.vae.decode(last_x0_pred, return_dict=False)[0]
            # image = self.image_processor.postprocess(image, output_type=output_type)
            image = self.decode_latents(last_x0_pred, height, width, output_type=output_type)

        # Offload all models
        self.maybe_free_model_hooks()

        # Always return packed latents + last x0 prediction (callers use decode_latents() to decode)
        if not return_dict:
    ...
    def decode_latents(self, x, height, width, output_type="pil"):
        x = self._unpack_latents(x, height, width, self.vae_scale_factor)
